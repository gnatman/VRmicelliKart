...
bool FreecamKeyDown(int virtualKey) {
    ...
    if (static_cast<Ship::WindowBackend>(wnd->GetWindowBackend()) == Ship::WindowBackend::FAST3D_SDL_OPENGL) {
        // Use SDL to check key states
        const uint8_t* keystate = SDL_GetKeyboardState(NULL);
        isDownNow = keystate[virtualKey] != 0;
    }
#ifdef _WIN32
    else if (static_cast<Ship::WindowBackend>(wnd->GetWindowBackend()) == Ship::WindowBackend::FAST3D_DXGI_DX11) {
...
#endif
    else if (static_cast<Ship::WindowBackend>(wnd->GetWindowBackend()) == Ship::WindowBackend::FAST3D_SDL_OPENGL) {
        const uint8_t* keystate = SDL_GetKeyboardState(NULL);
