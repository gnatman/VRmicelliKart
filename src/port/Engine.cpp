...
#include "enhancements/wheel/FFBEffects.h"
#include "enhancements/telemetry/Telemetry.h"
#include "enhancements/telemetry/TelemetryCalibration.h"
...
GameEngine::GameEngine() {
    ...
    auto controlDeck = std::make_shared<SpaghettiControlDeck>(std::vector<CONTROLLERBUTTONS_T>(), defaultMappings, buttonNames);
...
void GameEngine::Create() {
    ...
    Ship::FFBManager::GetInstance().Init(""); // TODO: Pass real GUID from CVar
...
void GameEngine::StartFrame() const {
    ...
    if (CVarGetInteger("gTelemetry.Enabled", 0)) {
        int playerIndex = CVarGetInteger("gTelemetry.PlayerIndex", 0);

        sTelemetryData data = {};
        data.sBase.mPacketType = 0;
        data.sBase.mPacketVersion = 2; // PC2 version

        // N64 tick is typically 30Hz
        TelemetryCalibration::Calibrate(data, &gPlayers[playerIndex], 0.0333f);
        Telemetry::GetInstance().PushData(data);
    }
    ...
    if (CVarGetInteger("gWheel.Enabled", 0)) {
        ...
        FFBEffects_Update(p, p->velocity[0], p->rotation[1], 0, 0);
    }

    if (VR_IsEnabled()) {
        int playerIndex = CVarGetInteger("gTelemetry.PlayerIndex", 0);
        float vignette = VR_GetVignetteIntensity(&gPlayers[playerIndex]);
        // TODO: Pass vignette intensity to a post-processing shader
    }
}
...
void GameEngine::RunCommands(Gfx* pool, const std::vector<std::unordered_map<Mtx*, MtxF>>& mtx_replacements) {
    ...
    if (VR_IsEnabled()) {
        VRPose pose;
        if (VRSession::GetInstance().WaitFrame(pose)) {
            VRSession::GetInstance().BeginFrame();
            for (int eye = 0; eye < 2; eye++) {
                VRSession::GetInstance().SetCurrentEye(eye);
                VRSession::GetInstance().RenderEye(eye, [&](ID3D11RenderTargetView* rtv) {
                    interpreter->SetVRFramebuffer(rtv);
                    
                    // Render current frame for this eye
                    // In a more advanced impl, we would interpolate using pose.displayTime
                    interpreter->mInterpolationIndex = mtx_replacements.size() - 1;
                    wnd->DrawAndRunGraphicsCommands(pool, mtx_replacements.back());
                });
            }
            VRSession::GetInstance().EndFrame();
            
            // Unbind VR framebuffer for desktop mirror
            interpreter->SetVRFramebuffer(nullptr);
        }
    }
