...
void DrawTelemetryMenu() {
    if (ImGui::BeginMenu("Telemetry")) {
        bool enabled = Ship::CVarGetInteger("gTelemetry.Enabled", 0);
        if (ImGui::Checkbox("Enabled", &enabled)) {
            Ship::CVarSetInteger("gTelemetry.Enabled", enabled);
            Ship::Context::GetInstance()->GetConfig()->Save();
        }

        float speedScale = Ship::CVarGetFloat("gTelemetry.SpeedScale", 0.05f);
        if (ImGui::SliderFloat("Speed Scale", &speedScale, 0.01f, 0.2f)) {
            Ship::CVarSetFloat("gTelemetry.SpeedScale", speedScale);
            Ship::Context::GetInstance()->GetConfig()->Save();
        }

        int rpmMax = Ship::CVarGetInteger("gTelemetry.RPMMax", 9500);
        if (ImGui::SliderInt("RPM Max", &rpmMax, 5000, 15000)) {
            Ship::CVarSetInteger("gTelemetry.RPMMax", rpmMax);
            Ship::Context::GetInstance()->GetConfig()->Save();
        }
...
void DrawWheelMenu() {
    if (ImGui::BeginMenu("Wheel")) {
        bool enabled = Ship::CVarGetInteger("gWheel.Enabled", 0);
        if (ImGui::Checkbox("Enabled", &enabled)) {
            Ship::CVarSetInteger("gWheel.Enabled", enabled);
            Ship::Context::GetInstance()->GetConfig()->Save();
        }

        float linearity = Ship::CVarGetFloat("gWheel.Linearity", 0.85f);
        if (ImGui::SliderFloat("Linearity", &linearity, 0.1f, 2.0f)) {
            Ship::CVarSetFloat("gWheel.Linearity", linearity);
            Ship::Context::GetInstance()->GetConfig()->Save();
        }

        float deadzone = Ship::CVarGetFloat("gWheel.Deadzone", 0.02f);
        if (ImGui::SliderFloat("Deadzone", &deadzone, 0.0f, 0.2f)) {
            Ship::CVarSetFloat("gWheel.Deadzone", deadzone);
            Ship::Context::GetInstance()->GetConfig()->Save();
        }
...
void DrawVRMenu() {
    if (ImGui::BeginMenu("VR")) {
        bool enabled = Ship::CVarGetInteger("gVR.Enabled", 0);
        if (ImGui::Checkbox("Enabled (Cockpit View)", &enabled)) {
            Ship::CVarSetInteger("gVR.Enabled", enabled);
            Ship::Context::GetInstance()->GetConfig()->Save();
        }

        float worldScale = Ship::CVarGetFloat("gVR.WorldScale", 1.0f);
        if (ImGui::SliderFloat("World Scale", &worldScale, 0.1f, 5.0f)) {
            Ship::CVarSetFloat("gVR.WorldScale", worldScale);
            Ship::Context::GetInstance()->GetConfig()->Save();
        }
