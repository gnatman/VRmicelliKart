# VRmicelliKart Development Plan

## Overview
This document outlines the phased implementation plan for adding VR, Racing Wheel, and Telemetry support to VRmicelliKart.
...
| **3. VR camera (no stereo)** | `VRMode` "camera-only" mode: first-person head anchor on kart, toggleable. Uses desktop 2D window. | Cockpit view on desktop confirms camera-override pipeline *before* touching rendering |
